...
export const TOKEN_ADDRESSES = {
    sepolia: {
        USDC: "0x8816c7e64F9d7d9AF4C999a41CC714f14bB961A2" as Address, // Your MockUSDC
        ...
        // Mock addresses for testing (replace with actual deployed addresses)
        TSLA: "0x0000000000000000000000000000000000000001" as Address, // MOCK - Tesla Inc Stock
        GOOGL: "0x0000000000000000000000000000000000000002" as Address, // MOCK - Alphabet Inc Stock
        NVDA: "0x0000000000000000000000000000000000000003" as Address, // MOCK - NVIDIA Corp Stock
        MSFT: "0x0000000000000000000000000000000000000004" as Address, // MOCK - Microsoft Corp Stock
    },
    arbitrumSepolia: {
...
export function getStockLendProtocolAddress(chainId: number): Address {
    ...
    if (!STOCK_LEND_PROTOCOL_ADDRESSES[chainId]) {
        console.warn(
            `Protocol address not defined for chainId ${chainId}, using fallback address`
        );
    ...
    if (address === "0x0000000000000000000000000000000000000000") {
        console.warn(
            `Protocol address is zero address for chainId ${chainId}, using fallback address`
        );
...
export function getTokenAddress(chainId: number, tokenSymbol: string): Address {
    ...
    console.log(
        `Getting token address for ${tokenSymbol} on network ${networkName} (chainId: ${chainId})`
    );
    ...
    if (!tokens[tokenSymbol as keyof typeof tokens]) {
        console.error(
            `Token ${tokenSymbol} not defined on network ${networkName}`
        );
        throw new Error(`Token ${tokenSymbol} not found on chain ${chainId}`);
    }
    ...
    // Check if this is a mock address (starts with 0x000...000 but not exactly zero address)
    if (
        address.startsWith("0x000000000000000000000000000000000000000") &&
        address !== "0x0000000000000000000000000000000000000000"
    ) {
        console.warn(
            `⚠️  Using mock address for ${tokenSymbol} on ${networkName}: ${address}`
        );
        console.warn(
            `⚠️  This token is not actually deployed. Replace with real address when available.`
        );
    }
